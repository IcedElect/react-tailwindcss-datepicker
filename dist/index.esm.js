...
const Days = ({ calendarData, onClickPreviousDays, onClickDay, onClickNextDays }) => {
    ...
    const activeDateData = useCallback((day) => {
        ...
        if (require$$0(fullDay).isSame(period.start) && require$$0(fullDay).isSame(period.end)) {
            ...
            className = `bg-accent-900/20 text-white font-medium ${require$$0(fullDay).isSame(dayHover) && !period.end
                ? "rounded-full"
                : "rounded-l-full"}`;
        }
        else if (require$$0(fullDay).isSame(period.end)) {
            className = `bg-accent-900/20 text-white font-medium ${require$$0(fullDay).isSame(dayHover) && !period.start
                ? "rounded-full"
                : "rounded-r-full"}`;
        }
        return {
...
const Datepicker = ({ primaryColor = "blue", value = null, onChange, useRange = true, showFooter = false, showShortcuts = false, configs = undefined, asSingle = false, placeholder = null, separator = "~", startFrom = null, i18n = LANGUAGE, disabled = false, inputClassName = null, containerClassName = null, toggleClassName = null, toggleIcon = undefined, displayFormat = DATE_FORMAT, readOnly = false, minDate = null, maxDate = null, dateLooking = "forward", disabledDates = null, inputId, inputName, startWeekOn = "sun", classNames = undefined, popoverDirection = undefined, controlComponent = null }) => {
    ...
    return (React.createElement(DatepickerContext.Provider, { value: contextValues },
        React.createElement("div", { className: containerClassNameOverload, ref: containerRef },
            controlComponent ? (React.createElement(controlComponent, {
...
export { DatepickerContext, Datepicker as default };
